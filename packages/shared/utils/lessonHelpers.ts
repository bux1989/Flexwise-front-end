// Shared lesson utility functions
...
export const getSubstituteLessons = (): SubstituteLesson[] => {
...
export const getMobileSubjectAbbreviation = (subject: string): string => {
...
export const getMobileTeacherAbbreviation = (teacherName: string): string => {
...
export const isLessonCurrent = (startTime: Date, endTime: Date): boolean => {
  const now = new Date();
  return now >= startTime && now <= endTime;
};

export const isLessonUpcoming = (startTime: Date): boolean => {
  const now = new Date();
