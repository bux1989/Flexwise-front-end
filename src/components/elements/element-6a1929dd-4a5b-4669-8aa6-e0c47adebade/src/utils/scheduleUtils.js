...
/**
 * Create optimized lookup maps for lessons to improve performance
 * @param {Array} lessons - Array of lesson objects
 * @param {'teacher'|'class'} viewMode - Current view mode
 * @returns {Object} Lookup maps for fast lesson retrieval
 */
export const createLessonLookupMaps = (lessons = [], viewMode = 'teacher') => {
  const lessonMap = new Map();
  const teacherLessonsCache = new Map();
  
  // Create lookup key format: "rowItem|dayName|blockNumber"
  const createKey = (rowItem, dayName, blockNumber) => 
    `${rowItem}|${dayName}|${Number(blockNumber)}`;
  
  // Build the maps
  lessons.forEach(lesson => {
    if (!lesson) return;
    
    const { day_name_de, block_number } = lesson;
    
    if (viewMode === 'class') {
      // For class view, use class_name as the row item
      if (lesson.class_name) {
        const key = createKey(lesson.class_name, day_name_de, block_number);
        lessonMap.set(key, lesson);
      }
    } else {
      // For teacher view, create entries for each teacher in teacher_names
      if (Array.isArray(lesson.teacher_names)) {
        lesson.teacher_names.forEach(teacherName => {
          const key = createKey(teacherName, day_name_de, block_number);
          lessonMap.set(key, lesson);
          
          // Build teacher lessons cache for workload calculations
          if (!teacherLessonsCache.has(teacherName)) {
            teacherLessonsCache.set(teacherName, []);
          }
          teacherLessonsCache.get(teacherName).push(lesson);
        });
      }
    }
  });
  ...
  return {
    lessonMap,
    teacherLessonsCache,
    getLessonByKey: (rowItem, dayName, blockNumber) => {
      const key = createKey(rowItem, dayName, blockNumber);
      return lessonMap.get(key) || null;
    },
    getTeacherLessons: (teacherName) => {
      return teacherLessonsCache.get(teacherName) || [];
    }
  };
};

/**
 * Optimized lesson lookup function using pre-computed maps
 * @param {Object} lookupMaps - Pre-computed lookup maps from createLessonLookupMaps
 * @param {String} rowItem - Teacher or class name
 * @param {String} dayName - Day name
 * @param {Number} blockNumber - Period block number
 * @returns {Object|null} Lesson object or null
 */
export const getOptimizedLesson = (lookupMaps, rowItem, dayName, blockNumber) => {
  if (!lookupMaps || !lookupMaps.getLessonByKey) return null;
  return lookupMaps.getLessonByKey(rowItem, dayName, blockNumber);
};

/**
 * Optimized teacher lesson count using cached data
 * @param {Object} lookupMaps - Pre-computed lookup maps from createLessonLookupMaps
 * @param {String} teacherName - Full teacher name
 * @returns {Number} Number of lessons for the teacher
 */
export const getOptimizedTeacherLessonCount = (lookupMaps, teacherName) => {
  if (!lookupMaps || !lookupMaps.getTeacherLessons) return 0;
  return lookupMaps.getTeacherLessons(teacherName).length;
};
