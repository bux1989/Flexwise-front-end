...
export default {
  ...
  setup(props, { emit }) {
    ...
    const getLesson = (rowItem, day, block) => {
      if (!activeLessons.value || !activeLessons.value.length) return null;
      if (viewMode.value === 'class') {
        return activeLessons.value.find(
          l => l && l.class_name === rowItem &&
            l.day_name_de === day &&
            Number(l.block_number) === Number(block)
        );
      } else {
        return activeLessons.value.find(
          l => l && Array.isArray(l.teacher_names) &&
    ...
    const getTeacherLessonCount = (teacherName) => {
      ...
      return (activeLessons.value || []).filter(lesson =>
        ...
        lesson.teacher_names.includes(teacherName)
      ).length;
    };
