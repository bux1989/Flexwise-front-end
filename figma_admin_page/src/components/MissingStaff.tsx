...
import { AlertTriangle, Info, ArrowRight, CheckCircle, Users } from 'lucide-react';
...
interface MissingStaffEntry {
  ...
  substituteName?: string; // Name of the substitute teacher
  status: 'offen' | 'geloest'; // offen = open, geloest = resolved
}
...
export function MissingStaff() {
  ...
  const staffData = {
    ...
    morgen: [
      ...
      },
      {
        ...
        classes: ['7A'],
        hasSubstitute: true,
        substituteName: 'Dr. Michael Weber',
        status: 'geloest' as const
      },
      {
        name: 'Martin Koch',
  ...
  const handleSubstituteClick = () => {
    // This would navigate to the substitute planning module
    ...
    <Card className="h-full">
      ...
      <CardContent>
        ...
        <Tabs 
          ...
          onValueChange={(tab) => {
            setActiveTab(tab);
            setShowAllEntries(false); // Reset pagination when switching tabs
