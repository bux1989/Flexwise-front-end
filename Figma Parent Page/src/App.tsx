...
import { SickReportModal } from './components/SickReportModal';
        ...
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block">
              <span className="text-lg text-gray-700">Hallo Father Müller</span>
            </div>
          </div>
                                    ...
                                    <>
                                      {!zeugnisSubmitted ? (
                                        <>
                                          ...
                                          {/* Individual Decision Mode */}
                                          {zeugnisDecisionMode === 'individual' && (
                                            <div className="space-y-4">
                                              <div className="flex items-center justify-between">
                                                <h4 className="font-medium text-gray-800">Entscheidung für jedes Kind:</h4>
                                                <Button
                                                  onClick={() => setZeugnisDecisionMode('global')}
                                                  variant="ghost"
                                                  size="sm"
                                                  className="text-blue-600 hover:text-blue-800 text-xs"
                                                >
                                                  ← Zurück zu gemeinsamer Entscheidung
                                                </Button>
                                              </div>
                                              
                                              {children.map((child) => {
                                                const currentDecision = zeugnisIndividualDecisions[child.id];
                                                return (
                                                  <div key={child.id} className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                                    <div className="flex items-center gap-2 mb-3">
                                                      ...
                                                      {currentDecision && (
                                                        <Badge 
                                                          variant="secondary" 
                                                          className={`text-xs ${
                                                            currentDecision.decision === 'early' 
                                                              ? 'bg-orange-100 text-orange-800' 
                                                              : 'bg-green-100 text-green-800'
                                                          }`}
                                                        >
                                                          {currentDecision.decision === 'early' ? '✓ Frühe Entlassung' : '✓ Normale Entlassung'}
                                                        </Badge>
                                                      )}
                                                    </div>
                                                    ...
                                                    <div className="space-y-3">
                                                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                        <Button
                                                          onClick={() => handleZeugnisIndividualDecision(child.id, 'early')}
                                                          variant={currentDecision?.decision === 'early' ? 'default' : 'outline'}
                                                          size="sm"
                                                          className={`text-xs h-auto p-2 ${
                                                            currentDecision?.decision === 'early' 
                                                              ? 'bg-orange-600 hover:bg-orange-700 text-white' 
                                                        ...
                                                        <Button
                                                          onClick={() => handleZeugnisIndividualDecision(child.id, 'normal')}
                                                          variant={currentDecision?.decision === 'normal' ? 'default' : 'outline'}
                                                          size="sm"
                                                          className={`text-xs h-auto p-2 ${
                                                            currentDecision?.decision === 'normal' 
                                                              ? 'bg-green-600 hover:bg-green-700 text-white' 
                                                              : 'border-green-200 text-green-700 hover:bg-green-50'
                                                          }`}
                                                        >
                                                      ...
                                                      {/* Pickup method for individual early release */}
                                                      {currentDecision?.decision === 'early' && (
                                                        <div className="p-2 bg-orange-50 border border-orange-200 rounded">
                                                          <p className="text-xs text-orange-800 mb-2 font-medium">Wie soll {child.name} um 13:00 Uhr entlassen werden?</p>
                                                          <div className="grid grid-cols-2 gap-2">
                                                            <Button
                                                              onClick={() => handleZeugnisIndividualDecision(child.id, 'early', 'pickup')}
                                                              variant={currentDecision?.pickupMethod === 'pickup' ? 'default' : 'outline'}
                                                              size="sm"
                                                              className={`text-xs h-auto p-1 ${
                                                                currentDecision?.pickupMethod === 'pickup' 
                                                                  ? 'bg-orange-600 hover:bg-orange-700 text-white' 
                                                                  : 'border-orange-300 text-orange-700 hover:bg-orange-100'
                                                              }`}
                                                            >
                                                            ...
                                                            <Button
                                                              onClick={() => handleZeugnisIndividualDecision(child.id, 'early', 'alone')}
                                                              variant={currentDecision?.pickupMethod === 'alone' ? 'default' : 'outline'}
                                                              size="sm"
                                                              className={`text-xs h-auto p-1 ${
                                                                currentDecision?.pickupMethod === 'alone' 
                                                                  ? 'bg-orange-600 hover:bg-orange-700 text-white' 
                                                                  : 'border-orange-300 text-orange-700 hover:bg-orange-100'
                                                              }`}
                                                            >
