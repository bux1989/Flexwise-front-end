-- Migration: Alternative fix for JSONB array handling in save_user_profile_complete_react function
-- Date: 2025-08-29
-- Issue: SQLSTATE 42804/42846 - JSONB array casting issues
-- Fix: Use CASE statements to handle NULL and non-NULL JSONB arrays properly
...
BEGIN
  ...
  -- Convert JSONB arrays to TEXT[] arrays safely
  v_skills := CASE 
    WHEN p_staff_data->'skills' IS NULL OR p_staff_data->'skills' = 'null'::jsonb THEN ARRAY[]::TEXT[]
    ELSE ARRAY(SELECT jsonb_array_elements_text(p_staff_data->'skills'))
  END;

  v_subjects_stud := CASE 
    WHEN p_staff_data->'subjects_stud' IS NULL OR p_staff_data->'subjects_stud' = 'null'::jsonb THEN ARRAY[]::TEXT[]
    ELSE ARRAY(SELECT jsonb_array_elements_text(p_staff_data->'subjects_stud'))
  END;
...
Uses CASE statements for safe JSONB array conversion to handle NULL values properly.
Parameters:
